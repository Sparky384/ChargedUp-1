...
public class DriveOnRamp extends CommandBase{
    ...
    public void execute() {
        ...
        if (backward == true) { //if we're going onto the ramp while facing backward.
            s_swerve.drive(new Translation2d(-0.35, 0).times(Constants.Swerve.maxSpeed).times(-1),
            0, 
            false, 
            true);
        }
        else { //if we're going onto the ramp while facing forward/towards it.
            s_swerve.drive(new Translation2d(0.35, 0).times(Constants.Swerve.maxSpeed),
            0, 
            false, 
            true);
        }
